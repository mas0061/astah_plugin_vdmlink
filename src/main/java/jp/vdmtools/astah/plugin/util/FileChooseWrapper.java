package jp.vdmtools.astah.plugin.util;
...
public class FileChooseWrapper {
  ...
  private void showDialog(int dialogType) {
    ...
    if (isJapanese()) {
      dialogTitle = (dialogType == FileDialog.LOAD ? J_LOAD_TITLE : J_SAVE_TITLE);
    } else {
      dialogTitle = (dialogType == FileDialog.LOAD ? E_LOAD_TITLE : E_SAVE_TITLE);
    }

    FileDialog fdlg = new FileDialog((Frame)mFrame, dialogTitle, dialogType);
  ...
  public boolean isFileSelected() {
    return StringUtils.isNotEmpty(mSelectFileDlg.getDirectory()) && StringUtils.isNotEmpty(mSelectFileDlg.getFile());
  }
